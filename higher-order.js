...
//Code Here
let postTaxPrices = prices.map((num, index, arr) => {
  return num * 1.07;
})
...
let bobsArray = purchases.filter((item) => item.owner === 'Bob');
let bobsTotal = bobsArray.reduce((total, item) => total + item.price, 0);
